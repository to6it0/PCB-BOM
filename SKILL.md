...
6. **Output — two files, same data.**
   - `<board name>_Sourcing.xlsx` with at least two sheets:
   ...
   - `<board name>_Dashboard.html` — an interactive companion view of the same data, generated by calling `generate_dashboard()` from `~/.claude/skills/bom/scripts/generate_dashboard.py`. Write a short per-board driver script (same pattern as the `.xlsx` builder) that imports this module and passes the board's `sourced_bom` and `supplier_rows` data — see the module's docstring for the exact schema (each `supplier_rows` entry needs a `links` dict of `{supplier_name: url}`, not per-supplier columns). Don't rebuild the HTML/CSS/JS by hand each time; the module is the reusable source of truth for this page's design and behavior (sortable/filterable tables, editable price/stock/best-price cells with localStorage persistence, light/dark theme). Publish it as a Claude Artifact too so the user gets a live preview link, in addition to the local file.
   - Both files go through the same one-file-per-board overwrite-in-place rule as step 4 — don't leave stale copies behind.
...
- **Dashboard generator**: `~/.claude/skills/bom/scripts/generate_dashboard.py` holds the HTML/CSS/JS for the interactive companion dashboard (self-contained, embedded fonts, works offline, no build step). Fonts are cached as base64 in `~/.claude/skills/bom/assets/fonts/` — don't re-fetch them from Google Fonts each run. Import the module's `generate_dashboard()` function from a per-board driver script rather than copying its HTML template; edit the module itself if the design needs to change so every board benefits, not just one board's copy.
- The dashboard's editable Price/Stock/Best-Price cells persist via the browser's localStorage, keyed per board name — this means edits are per-browser, not written back into the `.xlsx` or shared with anyone who opens the file elsewhere. Mention this limitation if the user asks about the edits' durability or portability.
